...
def fillmatrix(subpathDir,x,y,z,allDir):
    ...
    for suballDir in subpathDir:
            ...
            FirstEffectTag=True
            xLen=len(x)-1# the last x index
            for sentence in sentenceList:
                if not len(sentence.split())==0:
                    processLine(sentence,x, y, z)
                    if FirstEffectTag:
                        x[xLen+1]="BeginLun+"+x[xLen+1]
                        FirstEffectTag=False
                        
            
            x[len(x)-1]="EndLun+"+x[len(x)-1]#notice that sometimes the Begin tag and end tag may be in the same sentence, because a comment only have one comment.
