            ...
            if tagLi:
                ...
                try:
                    if not len(sentence.text.split())==0:
                        ...
                        file.write(" numDot "+sentence.text.strip().encode('ascii',errors='ignore')+os.linesep)
                ...
                try:
                    if not len(sentence.text.split())==0:
                        file.write(sentence.text.strip().encode('ascii',errors='ignore')+os.linesep)
                except:    
                    print("error symbol")
    except:
        try:
            file.write(line.strip().encode('ascii',errors='ignore')+os.linesep)#some special symbol
        except:
            print("error symbol")
...
def TrackerFolders(subpathDir, tokenizer, nlp):
        ...
        for suballDir in subpathDir:
            ...
            elif allDir=="bitbucket":
                tree = html.parse("recdroid-dataset/"+allDir+"/"+suballDir)
                ######every comments
                buyers=tree.xpath('//div[@class="issue-description wiki-content"] | //div[@class="wiki-content comment-content"]')
                ######li part
                liTag=tree.xpath('//div[@class="issue-description wiki-content"]/.//li | //div[@class="wiki-content comment-content"]/.//li')
                ######title part
                titleXpath=tree.xpath('//span[@id="issue-title"]')[0]
                
            
            elif allDir=="gitlab":
                tree = html.parse("recdroid-dataset/"+allDir+"/"+suballDir)
                ######every comments
                buyers=tree.xpath('//textarea[@class="hidden js-task-list-field"]')
                ######li part
                liTag=tree.xpath('//textarea[@class="hidden js-task-list-field"]/.//li')
                ######title part
                titleXpath=tree.xpath('//h2[@class="title"]')[0]
                
                
                
            
            ##################take title text
            titleText=h.handle(etree.tostring(titleXpath))
            ...
            for liItem in liTag:
                ...
                liTextlines=h.handle(etree.tostring(liItem))
            ...
            for buyer in buyers:#one buyer is one comment
                lineList=h.handle(etree.tostring(buyer)).splitlines()
