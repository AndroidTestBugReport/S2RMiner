...
def VectcountTrans(allTrainX, vector):
    ...
    countX=vector.fit_transform(extendVector).todense()
    
    #countX=np.array(vector.fit_transform(extendVector))#len(count[0])=21041
    countX=countX.reshape(-1,5,21041)
    #countX=np.array(vector.fit_transform(extendVector))
    
    #allTrainX=[]
    
    #allTrainX=np.empty((5,21041), int)
    ...
    allTrainX=np.empty((0,5,21041))
    
    
    
    for i in range(0,len(countX)/5):  # it should be 0 if from begin

        
        sent5=np.append([countX[5*i]],[countX[5*i+1]],0)
        sent5=np.append(sent5,[countX[5*i+2]],0)
        sent5=np.append(sent5,[countX[5*i+3]],0)
        sent5=np.append(sent5,[countX[5*i+4]],0)
        allTrainX=np.append(allTrainX,[sent5],0)
