...
if __name__ == '__main__':
    ...
    epochsVal=30
    if sys.argv[7]=="-epochs":
        epochsVal=int(sys.argv[8])
        print ("epoch:")
        print(epochsVal)
    else:
        print("wrong input")
    ...
    model.fit(allTrainX, allTrainY, epochs=epochsVal, batch_size=64, class_weight = 'auto')
    model.summary()
    ...
    outputFile.write("embeding_cnn_lstm\n")
    outputFile.write("Maxwordsent: "+str(Maxwordsent)+"\n")
    outputFile.write("vectorSize: "+str(vectorSize)+"\n")
    outputFile.write("epochs: "+str(epochsVal)+"\n")
    outputFile.write("target: "+str(target)+"\n")
