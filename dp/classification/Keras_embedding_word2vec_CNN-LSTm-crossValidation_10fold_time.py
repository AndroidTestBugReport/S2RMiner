...
if __name__ == '__main__':
    ...
    trainFolder='cross-valid'+'/train'
    #testFolder='cross-valid'+'/test'
    ...
    kfold = StratifiedKFold(n_splits=10000, shuffle=True, random_state=seed)
    for trainIndex, testIndex in kfold.split(allTrainX, allTrainY):
        ...
        model.fit(allTrainX[trainIndex], allTrainY[trainIndex], epochs=30, batch_size=64, class_weight = 'auto')
        model.summary()
        ...
        resultList=model.predict(allTrainX[testIndex])
        ...
        for i in range(0,len(resultList)-1):
            result=resultList[i]
            if allTrainY[testIndex][i]==1:#real positive
            ...
            elif allTrainY[testIndex][i]==0:#real negative
    ...
    #model.predict(X_train[0]) # this predict needs the first dimension of the X_train
        ...
        outputFile = open("cross-valid/result_test_muli_5fold_step_time.txt", 'a')
        outputFile.write("\n")
        outputFile.write("original embeding_cnn_lstm\n")
        outputFile.write("time b1-a1: "+str(b1-a1)+"\n")
        outputFile.write("time b-a: "+str(b-a)+"\n")
        outputFile.close()
