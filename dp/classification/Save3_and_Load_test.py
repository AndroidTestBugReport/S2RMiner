...
def extractAllTrain(allTrainX,allTrainY,allTrainZ, pathDir, Folder, allTestsent, TestBol):#### TestBol and sent5Text are just to use check FP and FN
    ...
    testCount=500######3this is just for test
...
if __name__ == '__main__':
    ...
    model.fit(allTrainX, allTrainY, epochs=2, batch_size=64, class_weight = 'auto')
    model.summary()
    
    
    
    model.save("keras_model_test.h5", overwrite=True)
    #####################test##############################
    del model
    
    mm=load_model("keras_model_step.h5")
    #mm=load_model("keras_model_test.h5")
    ...
    del allTrainX#sentence
    del allTrainY#step list
    del allTrainZ#crash list
    ...
    resultList=mm.predict(allTestX)
