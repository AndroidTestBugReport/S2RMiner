...
from keras.layers import Dense, Activation, Embedding, Dropout, TimeDistributed, Flatten, Bidirectional,Masking
...
if __name__ == '__main__':
    ...
    if sys.argv[2]=="-vesize":
        vectorSize=int(sys.argv[3])
    else:
        print("wrong input")
        exit()
        
    if sys.argv[4]=="-maxsize":
        Maxwordsent=int(sys.argv[5])
    else:
        print("wrong input")
        exit()  
    ...
    fname="./my_word2vec_model_"+sys.argv[3]
    ...
    if smoteBool:        
        allTrainX=allTrainX.reshape(-1,5*Maxwordsent)
    ...
    sents_input = Input(shape=(5,Maxwordsent), name='sents_input')#(,5,200)
    words = TimeDistributed(Embedding(input_dim=wordEmbeddings.shape[0], output_dim=wordEmbeddings.shape[1],  embeddings_initializer=Constant(wordEmbeddings), trainable=False))(sents_input)#(,5,200,50)
    mask_words=TimeDistributed(Masking(mask_value=0., input_shape=(Maxwordsent, vectorSize)))(words)
    
    dis_LSTM=TimeDistributed(Bidirectional(LSTM(vectorSize, dropout=0.50, recurrent_dropout=0.25)))(mask_words)
    dis_Dense_output=TimeDistributed(Dense(vectorSize,activation='relu'))(dis_LSTM)
    dis_Dropout_output=Dropout(0.5)(dis_Dense_output)
    dis_output=TimeDistributed(Dense(vectorSize,activation='relu'))(dis_Dropout_output)
    ...
    LSTM_output=Bidirectional(LSTM(vectorSize, dropout=0.50, recurrent_dropout=0.25))(dis_output)
    ...
    outputFile = open("result_test.txt", 'a')
    outputFile.write("\n")
    
    outputFile.write("embeding_lstm_lstm\n")
    outputFile.write("Maxwordsent: "+str(Maxwordsent)+"\n")
    outputFile.write("vectorSize: "+str(vectorSize)+"\n")
    outputFile.write("SMOTE: "+str(smoteBool)+"\n")
    outputFile.write("Precision:  "+str(P) +"\n")
    outputFile.write("Recall:  "+str(R) +"\n")
    outputFile.write("F score:  "+str(Fscore) +"\n")
