...
def writeFile(motherFolder,allDir,title,comments):
    fileOutput=open(motherFolder+"step-policy8/"+   allDir+"8", 'w')
    
    #for line in outputStep:
    for subList in comments:
        for line in subList:
            fileOutput.write(line.replace("numDot","").replace("#step","").strip()+os.linesep)###policy 5 change
...
def policy(motherFolder):
    ...
    for allDir in stepFolder:
        ...
        #print(comments)
        writeFile(motherFolder,allDir,title,comments)
