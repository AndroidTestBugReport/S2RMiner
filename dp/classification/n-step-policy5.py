...
def findMostStep(comments):
    outputStep=[]
    if len(comments)>0:
        outputStep=comments[0]###policy 5 change
...
def writeFile(motherFolder,allDir,title,outputStep):
    fileOutput=open(motherFolder+"step-policy5/"+   allDir+"5", 'w')
    
    for line in outputStep:
        fileOutput.write(line.replace("#step","").replace("numDot","").strip()+os.linesep)###policy 5 change
