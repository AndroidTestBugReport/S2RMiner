...
if __name__ == '__main__':
    ...
    words = TimeDistributed(Embedding(input_dim=wordEmbeddings.shape[0], output_dim=wordEmbeddings.shape[1],  embeddings_initializer=Constant(wordEmbeddings), mask_zero=True, trainable=False))(sents_input)#shape[0]is the vacoabilry size, shape[1] is output size #embedding_1 (Embedding)         (None, None, 100)  the first None is the batch, the second None is the number of word in a sentence
    #mask_words=TimeDistributed(Masking(mask_value=0., input_shape=(Maxwordsent, vectorSize)))(words)
    
    #mask_words=TimeDistributed(Masking(mask_value=np.zeros(vectorSize), input_shape=(Maxwordsent, vectorSize)))(words)
    #model=Model(inputs=[sents_input],outputs=[sents_input])
    model=Model(inputs=[sents_input],outputs=[words])
    ...
    intermediate_output = model.predict(allTrainX)
    ...
    #model.fit(allTrainX, allTrainY, epochs=1, batch_size=64, class_weight = 'auto')
    #model.summary()    
