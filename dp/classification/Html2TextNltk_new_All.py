...
def divdieAndWrite(line,tagLi,file, tokenizer):
    ...
    if "Look revisions in another project" in line:
        print("bingo")
...
def TrackerFolders(subpathDir, tokenizer):
        ...
        for suballDir in subpathDir:
            ...
            treeold= html.parse("newTrainingSetWhole/google code/wholeId-17-page-1-appid-4.html")
                        
            #treeold = html.parse("newTrainingSetWhole/"+allDir+"/"+suballDir)
            htmlString=str(etree.tostring(treeold), 'ascii', 'ignore')
            parser = etree.HTMLParser()
            '''
            clearner=Cleaner(page_structure=True,
                              meta=True,
                              embedded=True,
                              links=True,
                              style=True,
                              ...
                              remove_tags=('strong'))
            #htmlString=clearner.clean_html(htmlString)
            '''
            ...
            #clean6 =re.compile('<a.*?>')
            clean0 =re.compile('<code.*?>')
            clean1 =re.compile('<\code.*?>')
            clean2 =re.compile('<em.*?>')
            clean3 =re.compile('<\em.*?>')
            clean4 =re.compile('<strong.*?>')
            clean5 =re.compile('<\strong.*?>')

            #htmlString=re.sub(clean0, '', htmlString)
            htmlString=re.sub(clean0, '', htmlString)
            htmlString=re.sub(clean1, '', htmlString)
            htmlString=re.sub(clean2, '', htmlString)
            htmlString=re.sub(clean3, '', htmlString)
            htmlString=re.sub(clean4, '', htmlString)
            htmlString=re.sub(clean5, '', htmlString)
            #htmlString=re.sub(clean6, '', htmlString)
            
            
            
            
            tree=etree.fromstring(htmlString, parser=parser)
