...
if __name__ == '__main__':
    ...
    words = TimeDistributed(Embedding(input_dim=wordEmbeddings.shape[0], output_dim=wordEmbeddings.shape[1],  embeddings_initializer=Constant(wordEmbeddings), mask_zero=True, trainable=False))(sents_input)#(,5,200,50)
    #mask_words=TimeDistributed(Masking(mask_value=0., input_shape=(Maxwordsent, vectorSize)))(words)
    
    dis_LSTM=TimeDistributed(Bidirectional(LSTM(vectorSize, dropout=0.50, recurrent_dropout=0.25)))(words)
    ...
    outputFile = open("result_test_original.txt", 'a')
    outputFile.write("\n")
    outputFile.write("original embeding_lstm_lstm\n")
